...
def normalize(value: str, mode: str) -> str:
    """Normalize a value based on mode: 'title', 'artist', or 'genre'."""
    if mode == "title":
...
def normalize_song(raw: Song) -> Song:
    ...
    title = normalize(str(raw.get("title", "")), mode="title")
    artist = normalize(str(raw.get("artist", "")), mode="artist")
    genre = normalize(str(raw.get("genre", "")), mode="genre")
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # compute total using all songs rather than just hype
    total = len(all_songs)  # was len(hype) which made ratios always 1.0 when hype nonempty
    # ratio of hype songs to total songs
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        # sum energy across every song, not just hype
        total_energy = sum(song.get("energy", 0) for song in all_songs)
        # divide by the total number of songs
